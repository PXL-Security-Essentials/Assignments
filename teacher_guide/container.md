...
Container challenges are the most complex but also the most impactful feature of our platform. To get started, we'll build a Docker container on our local machine that we can use on our platform later. We'll need Docker installed and a Docker Hub account to which we can push our container.

Note: All containers must be publicly available on Docker Hub.

## Creating the container

Create a new folder on your machine called `seclabs` and, in that folder, create a new folder called `html`. In this `html` folder, create a new `index.html` file with the following contents:
```html
<!DOCTYPE html>
<html lang="en">
  <head>
...
The environment variables in the Dockerfile (`FLAG1`, `FLAG2`) are just placeholders. The SecLabs platform will override these with unique values for each team when containers are started.
...
exec "$@"
```
Tip: Make sure your IDE uses LF rather than CRLF (Windows), or your container might crash.
